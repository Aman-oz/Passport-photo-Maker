...
class GetDocumentsByType @Inject constructor(
    private val documentRepository: DocumentRepository,
    private val insertSeparatorIntoPagingData: InsertSeparatorIntoPagingData
) {

    fun documents(type: String, pageSize: Int): Flow<PagingData<DocumentListItem>> = documentRepository.documents(type,pageSize).map {
        val pagingData: PagingData<DocumentListItem.Document> = it.map { document ->
            Logger.d("GetDocumentsByType", "Mapping document: ${document.name}")
